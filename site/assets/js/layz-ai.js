...
(function () {
  ...
  async function fetchCopy(lz, mode, answers, questions) {
    var layzSystem = [
      ...
      'head·desc·recs·rx 전 구간이 같은 「오늘의 한 사람」 이야기여야 합니다.',
  ...
  function mapToUi(ai, mode, resultCopy, getResultDetails) {
    ...
    if (ai.rx && Array.isArray(ai.rx.items) && ai.rx.items.length >= 3) {
      rx = {
        title: String(ai.rx.title || '오늘의 수면 가이드').trim(),
    ...
    return {
      ...
      desc: String(ai.desc || copy.desc).trim(),
      tags: String(ai.tierTag || '')
        ...
        .concat(copy.tags || [])
        .slice(0, 4),
      summary: String(ai.desc || fallback.summary).trim(),
